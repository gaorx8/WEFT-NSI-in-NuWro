...
class event:public TObject
{
	public:
		...
		inline bool select();
		inline double pn ();
		inline double costheta_muon ();
		inline double delta_p_t();
		inline double delta_alpha_t();
		inline bool delta_alpha_t(int k);
		inline double alpha_t ();
		inline double mu_out ();
		inline double mu_post ();
		inline double out_pz (int k);
		inline double post_pz (int k);
		inline double p_out ();
		inline double p_post ();
		ClassDef (event, 1);
};
...
// 		if (post[k].proton()) {
// 			proton_number++;
// 			proton_k.push_back(k);
// 		}
// 	}

// 	double muon_angle = 0;
// 	double proton_angle = 0;

// 	if (pion_number == 0 && muon_number == 1 && proton_number >= 1) {

// 		if (post[muon_k].p().length() > 1500 && post[muon_k].p().length() < 10000) {
// 			muon_angle = acos(post[muon_k].p().z / post[muon_k].p().length());
// 			muon_angle = muon_angle * (180.0 / 3.14159);

// 			if (muon_angle < 20) {

// 				for (size_t i = 0; i < proton_k.size(); ++i) {

// 					if (post[proton_k[i]].p().length() > 450 && post[proton_k[i]].p().length() < 1200) {
// 						proton_angle = acos(post[proton_k[i]].p().z / post[proton_k[i]].p().length());
// 						proton_angle = proton_angle * (180.0 / 3.14159);
...
bool event::select()
{
	bool muon_check = false;
	int proton_in_range_number = 0;
	bool proton_check = false;

	for (int i = 0; i < post.size(); i++) {
		if (post[i].pdg == 111) {
			return false;
		}
		if (post[i].pdg == 211 || post[i].pdg == -211) {
			if (post[i].p().length() > 70 ) {
				return false;			
			}
		}

		if (post[i].muon()) {
			if (post[i].p().length() > 100 && post[i].p().length() < 1200) {
				muon_check = true;				
			}
		}

		if (post[i].proton()) {
			if (post[i].p().length() > 300 && post[i].p().length() < 1000) {
				proton_in_range_number++;
			}
		}
	}

	if (proton_in_range_number == 1) {
		proton_check = true;
	}
	
	if (muon_check && proton_check) {
		return true;
	}
	return false;
}

double event::costheta_muon ()
{
	for (int k = 0; k < post.size(); k++) {
		if (post[k].muon()) {
			return cos(vec(in[0]),vec(post[k]));			
		}
	}
...
double event::delta_p_t()
{
	vec delta_p_t(0,0,0);
	vec p_mu_t(0,0,0);
	vec p_p_t(0,0,0);
	double delta_p_t_mag;

	for (int i = 0; i < post.size(); i++) {
		if (post[i].muon()) {
			if (post[i].p().length() > 100 && post[i].p().length() < 1200) {
				p_mu_t = vec(post[i].x, post[i].y, 0);			
			}
		}		

		if (post[i].proton()) {
			if (post[i].p().length() > 300 && post[i].p().length() < 1000) {
				p_p_t = vec(post[i].x, post[i].y, 0);
			}
		}
	}

	delta_p_t = p_mu_t + p_p_t;
	delta_p_t_mag = delta_p_t.length() / 1000;

	return delta_p_t_mag;
}
...
double event::pn ()
{
	...
	for (int k = 0; k < post.size(); k++) {
		if (post[k].muon()) {
			vec p_mu_k_t(post[k].x, post[k].y, 0);
			p_mu_t = p_mu_t + p_mu_k_t;
			p_mu_l = post[k].p().z;
			E_mu = post[k].E();
		}

		if (post[k].proton()) {
			vec p_p_k_t(post[k].x, post[k].y, 0);
			p_p_t = p_p_k_t;
			p_p_l = post[k].p().z;
			E_p = post[k].E();
		}
	...
	vec p_t = p_mu_t + p_p_t;	
	double m_A = 68006.36;
	double m_n = 939.57;
	double b = 27.13;
	double m_A_prime = m_A - m_n + b;

	double R = m_A + p_mu_l + p_p_l - E_mu - E_p;
	double p_l = 0.5 * R - (m_A_prime * m_A_prime + p_t * p_t) / 2 / R;
	double pn = sqrt(p_t * p_t + p_l * p_l);

	return pn;
}

// transverse boosting angle
double event::alpha_t ()
{
	vec p_mu_t(0, 0, 0);
	vec p_p_t(0, 0, 0);
	double p_mu_l = 0;
	...
	double p_mu_t_mag = p_mu_t.length();
	double p_t_mag = p_t.length();
	double alpha_t = acos(- p_mu_t * p_t / p_mu_t_mag / p_t_mag);

	return alpha_t;
}

double event::delta_alpha_t ()
{
	vec delta_p_t(0,0,0);
	vec p_mu_t(0,0,0);
	vec p_p_t(0,0,0);
	...
	double delta_alpha_t = acos(-cos(p_mu_t, delta_p_t));

	delta_alpha_t = delta_alpha_t * 180.0 / 3.14159;

	return delta_alpha_t;
}

bool event::delta_alpha_t (int k)
{
	vec delta_p_t(0,0,0);
	vec p_mu_t(0,0,0);
	vec p_p_t(0,0,0);
	...
	bool select_alpha_t = 0;

	switch (k) 
	{
	case 1:
		if (delta_alpha_t < 45.0 * 3.14159 / 180.0) {
			select_alpha_t = 1;
		}
		break;
	case 2:
		if (delta_alpha_t > 45.0 * 3.14159 / 180.0 && delta_alpha_t < 90.0 * 3.14159 / 180.0) {
			select_alpha_t = 1;
		}
		break;
	case 3:
		if (delta_alpha_t > 90.0 * 3.14159 / 180.0 && delta_alpha_t < 135.0 * 3.14159 / 180.0) {
			select_alpha_t = 1;
		}
		break;
	case 4:
		if (delta_alpha_t > 135.0 * 3.14159 / 180.0 && delta_alpha_t < 180.0 * 3.14159 / 180.0) {
			select_alpha_t = 1;
		}
		break;
	default:
...
// fsi test
double event::mu_post ()
{
	return post[0].p().z;
}

// fsi test
double event::out_pz (int k)
{
	return out[k].p().z;
}

// fsi test
double event::post_pz (int k)
{
	return post[k].p().z;
}
...
double event::p_out ()
{
	...
	for (int k = 0; k < out.size(); k++) {
		if (out[k].proton()) {
			count++;
			cout << "Out Number:" << k << "is proton!!!!!!!!!!!!!!!!!!!!!!! \"" << endl;
			cout << "Out Count:" << count << endl;
		}
	}
...
double event::p_post ()
{
	...
	for (int k = 0; k < post.size(); k++) {
		if (post[k].proton()) {
			count++;
			cout << "Post Number:" << k << "is proton!!!!!!!!!!!!!!!!!!!!!!! \"" << endl;
