...
//		cout<<mat.Z<<' '<<mat.N<<' '<<endl;
	...
	switch (dyn)
	{
		case 0: 
			...
			{
				if (p.sf_weft) {
					sfevent_weft (p, *e, *_nucleus);
				} else if(p.sf_method>0 and has_sf(*_nucleus, p.sf_method)) {
...
// void  NuWro::load_data(const string& file, map<double, map<double, double>>& data_map) {
  ...
  shhpythiaitokay_();
	try
	{
		...
		else
		{
			...
			{
				// map<double, map<double, double>> data_map;
				// bool data_loaded = false;
				// if (!data_loaded) {
				// 	load_data("prob.txt", data_map);
				// 	data_loaded = true;
				// 	// std::cout << "PROB.TXT LOADED!!!!!!!!!!! " << std::endl;
				// }
